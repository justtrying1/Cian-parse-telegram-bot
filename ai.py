...
def send_request(desc, data, type, telegram=False):
    ...
    if type == 1:
        ...
        if telegram:
            stroka = """
                Если текст о сдаче квартиры а не комнаты в квартире, то пиши стоимость месячной аренды "не указано"
                ...
                Информацию о метро (расстояние и станция), о наличии залога, расстоянию до метро и стоимости месячной аренды. Если в тексте чего-либо не указано пиши "не указано". Если текст не о сдаче комнаты а о поиске комнаты, если написано "сниму комнату" пиши цена "не указано" Вот сам текст объявления: """
    if type == 3:
        stroka = """Привет, """
    proxies = {
