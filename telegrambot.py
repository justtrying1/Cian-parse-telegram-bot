...
def main():
    ...
    def start(message):
        ...
        keyboard.add(button_bar)
        bot.send_message(message.chat.id, "Привет, я бот, который с помощью искуственного интеллекта, будет обрабатывать описания новые объявлений на *Циане (по Москве)* и присылать те, которые подходят под ваш запрос (например, возможности проживания с кошкой) *в течение 3-х минут* после их публикации.\n\nПроект находится на стадии разработки, поэтому могут быть ошибки и, поэтому *для поиска доступны только комнаты!*\n\nДля начала нужно заполнить анкету, приступим?", reply_markup=keyboard, parse_mode= 'Markdown')
